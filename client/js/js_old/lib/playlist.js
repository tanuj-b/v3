...
_V_.Playlist = _V_.Component.extend({
  ...
  createElement: function() {
    ...
    for (i in this.videos) {
      //The strategy will be to selectively add thumbs. Also, an event for each video played.
      //If it is one with a thumb, it goes green.
      //
      var thumb = new _V_.PlaylistThumb(this.player, this.videos[i], i) //creating a new thumb here
      this.wrapperEl.appendChild(thumb.el); //adding it to dom
    };
  ...
  calculateWrapperWidth: function() {
    return (this.videos.length * 155); //155 pixels per video
  },
